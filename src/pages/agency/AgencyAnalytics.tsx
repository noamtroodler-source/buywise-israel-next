                ...
                <CardContent>
                  {!analytics?.agentPerformance?.length ? (
                    <div className="py-8 text-center">
                      ...
                      <p className="font-medium text-foreground mb-1">No agents with listings</p>
                      <p className="text-sm text-muted-foreground">
                        ...
                        {analytics.agentPerformance.map((agent, index) => {
                          ...
                          return (
                            <motion.div 
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              transition={{ delay: index * 0.05 }}
                              className="grid grid-cols-2 sm:grid-cols-6 gap-4 px-4 py-4 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors"
                            >
                              <div className="col-span-2 flex items-center gap-3">
                                <div className="h-10 w-10 rounded-xl bg-muted overflow-hidden flex-shrink-0">
                                  {agent.avatarUrl ? (
                                    <img 
                                      src={agent.avatarUrl} 
                                      alt={agent.agentName}
                                    ...
                                    <div className="h-full w-full flex items-center justify-center text-sm font-medium bg-primary/10 text-primary">
                                      {agent.agentName.charAt(0).toUpperCase()}
                                    </div>
                                  )}
                                </div>
                              ...
                              {/* Mobile stats */}
                              <div className="col-span-2 sm:hidden flex items-center gap-4 text-sm text-muted-foreground">
                                <span>{agent.activeListings} listings</span>
                                <span>{agent.views} views</span>
                                <span>{agent.saves} saves</span>
                              </div>
                            </motion.div>
                          );
                        })}
