  ...
  useEffect(() => {
    if (developerProfile) {
      updateData({
        ...
        authorProfileId: developerProfile.id,
      });
    }
  }, [developerProfile, updateData]);
    ...
    try {
      if (savedPostId) {
        ...
        const result = await createPost.mutateAsync({
          ...
          author_profile_id: developerProfile.id,
        });
...
export default function DeveloperBlogWizard() {
  const { id } = useParams();
  const isEditMode = !!id;
