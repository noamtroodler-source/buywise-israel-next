...
interface CompareProjectWinnerSummaryProps {
  projects: CompareProject[];
  winnerCounts: WinnerCount[];
}

export function CompareProjectWinnerSummary({ projects, winnerCounts }: CompareProjectWinnerSummaryProps) {
  if (projects.length < 2 || winnerCounts.length === 0) return null;
  ...
  // Don't show if no one has any wins
  if (leader.wins === 0) return null;
