...
export function StepContent() {
  ...
  const generateSummary = async () => {
    if (wordCount < 50) {
      toast.error('Please write at least 50 words before generating a summary');
      return;
    }
    setIsGeneratingSummary(true);
    try {
      const { data: result, error } = await supabase.functions.invoke('format-blog-content', {
        body: { content: data.content, mode: 'summary' }
      });
      if (error) throw error;
      if (result.error) throw new Error(result.error);
      updateData({ excerpt: result.formattedContent });
              ...
              {isFormatting ? (
                ...
                <>
                  ...
                  <div className="flex gap-3 mt-4 pt-4 border-t border-primary/20">
                    <Button type="button" onClick={applyFormatted} className="gap-2">
                      <Check className="h-4 w-4" />
                      Apply
                    </Button>
                    <Button type="button" variant="outline" onClick={dismissFormatted} className="gap-2">
                      <X className="h-4 w-4" />
                      Dismiss
                    </Button>
                  </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            disabled={isGeneratingSummary || wordCount < 50}
