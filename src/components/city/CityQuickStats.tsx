...
export function CityQuickStats({ citySlug, cityData, dataSources, lastVerified }: CityQuickStatsProps) {
  ...
  const getRentalPriceRange = () => {
    const getRentalForRooms = (rooms: number) => {
      ...
      if (rooms === 5 && cityData?.rental_5_room_min && cityData?.rental_5_room_max) {
        return { min: cityData.rental_5_room_min, max: cityData.rental_5_room_max };
      }
      return null;
    };

    const range = getRentalForRooms(selectedRooms);
    if (range) {
  ...
  const getYieldDisplay = () => {
    const grossYield = cityData?.gross_yield_percent;
    ...
    >
      <div className="container">
        <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 py-5 text-center">
          {/* Price per sqm */}
          {(priceRangeDisplay || formatPrice(pricePerSqm)) && (
            <div className="flex items-center gap-2">
              <span className="text-lg font-semibold text-foreground">
                {priceRangeDisplay || formatPricePerArea(pricePerSqm, 'ILS')}
              </span>
              <span className="text-sm text-muted-foreground">
                {priceRangeDisplay ? 'range' : 'avg'}
              </span>
            </div>
          )}
