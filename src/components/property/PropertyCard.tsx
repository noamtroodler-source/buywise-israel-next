...
import { SourcedListingImage } from './SourcedListingImage';
...
const PropertyCardComponent = memo(forwardRef<HTMLAnchorElement, PropertyCardProps>(function PropertyCard({ property, className, showCompareButton = false, showShareButton = true, showMonthlyEstimate = false, hideStatusBadge = false, compact = false, maxBadges = 2, showCategoryBadge = false, hideFeaturedBadge = false, compareCategory, alwaysShowCompare = false, hideTitle = false }, ref) {
  ...
  // Sourced (scraped) listings that haven't been claimed must not show scraped photos.
  // We use Street View / neighborhood gradient instead — legally clean alternatives.
  const isSourced = !!(property as any).import_source && !(property as any).is_claimed;

  // Israeli room system: source_rooms includes living room, bedrooms = source_rooms - 1.
  // Fall back to source_rooms calculation when explicit bedroom count is missing or zero.
  const sourceRooms = (property as any).source_rooms as number | null | undefined;
  const additionalRooms = (property as any).additional_rooms as number | null | undefined;
  const derivedBedrooms: number | null =
    property.bedrooms > 0
      ? property.bedrooms
      : sourceRooms != null && sourceRooms > 0
      ? Math.max(0, sourceRooms - 1)
      : null;
  const hasBathrooms = property.bathrooms != null && (property.bathrooms as unknown) !== '' && !isNaN(Number(property.bathrooms));
            ...
            <>
              <div className="relative aspect-[4/3] overflow-hidden rounded-t-xl" {...touchHandlers}>
                ...
                <div className={cn("absolute top-2 flex gap-1.5 z-10", alwaysShowCompare && compareCategory ? "left-10" : "left-2")}>
                  {(() => {
                    ...
                    // Partner badge for directly-listed onboarded agency listings
                    if (!(property as any).import_source && property.agent?.agency) {
                      badges.push(<PartnerBadge key="partner" compact />);
                    }
                    // Aggregated badge for unclaimed scraped listings
              ...
              <div className="p-3 pt-2.5 pb-3 bg-white border-t border-black/5 space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {[
                    derivedBedrooms != null
                      ? `${derivedBedrooms}${additionalRooms ? ` + ${additionalRooms}` : ''} bd`
                      : null,
                    hasBathrooms ? `${property.bathrooms} ba` : null,
                ...
                {/* Source attribution — only for unclaimed scraped listings */}
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  {derivedBedrooms != null && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs">{derivedBedrooms}{additionalRooms ? ` + ${additionalRooms}` : ''}</span>
                    </div>
                  )}
                  {hasBathrooms && (
                    <div className="flex items-center gap-1">
