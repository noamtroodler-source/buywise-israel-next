/**
 * BuyWise Israel — SSR Meta Injection Server
 *
 * Runs on Vercel (Node.js runtime). For the 5 SEO-critical routes it fetches
 * data from Supabase and injects proper <title>, <meta>, <og:*> and JSON-LD
 * tags into the built Vite SPA HTML before sending it to the browser.
 *
 * Every other route gets the plain SPA shell (existing behaviour).
 * Real users never notice the difference — the SPA still boots and hydrates.
 * Googlebot now sees real, crawlable HTML content.
 */
...
// ─── Supabase (anon/public key — reads published rows only) ──────────────────
...
// ─── HTML template (read once at cold-start) ─────────────────────────────────
...
try {
  ...
  // Fallback during local dev when dist/ doesn't exist yet
  templateHtml = '<html><head><!--SSR_META_START--><!--SSR_META_END--></head><body><div id="root"></div></body></html>'
}
...
/** Compact price: 4200000 → ₪4.2M, 9500 → ₪9,500 */
...
/** Replace the SSR_META block in the HTML template */
function injectMeta(
  title: string,
  description: string,
  canonicalUrl: string,
...
function sendHtml(res: Response, html: string) {
  ...
  // Cache for 10 minutes on CDN, 1 minute stale-while-revalidate
...
app.use(
  express.static(distPath, {
    maxAge: '1y',         // assets have hashed filenames — safe to cache forever
    immutable: true,
    index: false,         // we handle index routing ourselves below
  }),
)
...
// ─── SSR Route: Property detail ───────────────────────────────────────────────
app.get('/property/:id', async (req: Request, res: Response) => {
  try {
    const { data: p } = await supabase
      .from('properties')
      .select('id, title, bedrooms, size_sqm, price, currency, city, neighborhood, property_type, listing_status, description, condition, features, featured_highlight, images, is_furnished')
      .eq('id', req.params.id)
...
app.get('/areas/:slug', async (req: Request, res: Response) => {
  try {
    ...
    if (city.average_price_sqm) description += ` Average ₪${Math.round(city.average_price_sqm).toLocaleString()}/sqm.`
...
app.get('/blog/:slug', async (req: Request, res: Response) => {
  try {
    ...
    if (!description) description = `Read ${post.title} on BuyWise Israel. Expert insights on Israeli real estate.`
    if (description.length > 160) description = description.slice(0, 157) + '...'
    ...
    sendHtml(res, injectMeta(title, description, `${SITE_URL}/blog/${post.slug}`, post.cover_image, jsonLd))
  } catch {
    fallback(res)
  }
})

// ─── SSR Route: Project detail ────────────────────────────────────────────────
app.get('/projects/:slug', async (req: Request, res: Response) => {
  try {
    const { data: project } = await supabase
      .from('projects')
      .select('id, name, slug, city, neighborhood, description, status, total_units, price_from, price_to, currency, completion_date, images, developer:developers(name)')
      .eq('slug', req.params.slug)
      .single()
    ...
    const priceFrom = project.price_from ? `from ${compactPrice(project.price_from, project.currency ?? 'ILS')}` : ''
    ...
    sendHtml(res, injectMeta(title, description, `${SITE_URL}/projects/${project.slug}`, (project.images as string[] | null)?.[0], jsonLd))
  } catch {
    fallback(res)
  }
})
