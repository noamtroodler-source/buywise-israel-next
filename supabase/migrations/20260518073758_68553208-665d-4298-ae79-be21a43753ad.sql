...
SET
  ...
  city = 'Tel Aviv',
  neighborhood = CASE
    WHEN prepared.rn <= 3 THEN prepared.neighborhoods[((prepared.rn - 1) % array_length(prepared.neighborhoods, 1)) + 1]
    WHEN prepared.rn % 4 IN (0, 1) THEN NULL
  ...
  bathrooms = CASE
    WHEN prepared.rn <= 3 THEN 1 + (prepared.rn % 3)
    WHEN prepared.rn % 6 IN (0, 1) THEN NULL
    ELSE 1 + (prepared.rn % 3)
  END,
  size_sqm = CASE
    WHEN prepared.rn <= 3 THEN (68 + (prepared.rn * 8))::numeric
    WHEN prepared.rn % 5 IN (0, 2) THEN NULL
    ELSE (48 + (prepared.rn * 4))::numeric
  END,
  floor = CASE
    WHEN prepared.rn <= 3 THEN prepared.rn + 1
    WHEN prepared.rn % 7 IN (0, 3) THEN NULL
    ELSE prepared.rn % 18
  END,
  total_floors = 18,
...
UPDATE public.listing_agency_reviews r
SET
  status = 'needs_review',
  reviewed_at = NULL,
  reviewed_by = NULL,
