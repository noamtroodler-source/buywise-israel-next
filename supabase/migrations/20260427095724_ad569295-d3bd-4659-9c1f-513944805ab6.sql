...
BEGIN
  ...
  SELECT id, name, management_status INTO v_agency
  ...
  SELECT COALESCE(array_agg(DISTINCT id), ARRAY[]::uuid[]) INTO v_property_ids
  ...
  IF array_length(v_property_ids, 1) IS NOT NULL THEN
    ...
    DELETE FROM public.guest_property_saves WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.property_views WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.recently_viewed WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.outbound_clicks WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.image_hashes WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.listing_quality_flags WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.listing_reports WHERE property_id = ANY(v_property_ids);
    DELETE FROM public.inquiries WHERE property_id = ANY(v_property_ids);
    ...
    DELETE FROM public.import_conflicts WHERE property_id = ANY(v_property_ids) OR agency_id = p_agency_id;
    DELETE FROM public.import_job_items WHERE property_id = ANY(v_property_ids);
    ...
    DELETE FROM public.duplicate_pairs WHERE property_a = ANY(v_property_ids) OR property_b = ANY(v_property_ids) OR merged_into = ANY(v_property_ids);
    DELETE FROM public.merge_events WHERE loser_property_id = ANY(v_property_ids) OR winner_property_id = ANY(v_property_ids);
  ...
  UPDATE public.agency_sources SET last_sync_job_id = NULL WHERE agency_id = p_agency_id;
  UPDATE public.yad2_scrape_queue SET last_job_id = NULL WHERE agency_id = p_agency_id;
