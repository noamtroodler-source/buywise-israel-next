...
BEGIN
  ...
  IF array_length(v_property_ids, 1) IS NOT NULL THEN
    ...
    DELETE FROM public.cross_agency_conflicts WHERE attempted_property_id = ANY(v_property_ids) OR existing_property_id = ANY(v_property_ids) OR attempted_agency_id = p_agency_id OR existing_agency_id = p_agency_id;
    DELETE FROM public.duplicate_pairs WHERE property_a_id = ANY(v_property_ids) OR property_b_id = ANY(v_property_ids);
  ...
  IF array_length(v_agent_ids, 1) IS NOT NULL THEN
    UPDATE public.projects SET agent_id = NULL WHERE agent_id = ANY(v_agent_ids);
